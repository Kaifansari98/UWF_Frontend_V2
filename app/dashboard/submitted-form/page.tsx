...
export default function SubmittedAcknowledgementPage() {
  ...
  const [formToAccept, setFormToAccept] = useState<string | null>(null);
  ...
  const handleAcceptConfirm = async () => {
    if (!formToAccept) return;
    try {
      await apiClient.put(`/acknowledgement/accept/${formToAccept}`);
      toast.success("Acknowledgement accepted and case closed.");
      fetchSubmittedAcknowledgements();
    } catch (err: unknown) {
      const message =
        (err as { response?: { data?: { message?: string } } })?.response?.data?.message ||
        "Failed to accept acknowledgement.";
      toast.error(message);
    } finally {
    ...
    <div className="flex min-h-full flex-col bg-background text-foreground">
        ...
        <div className="flex items-center gap-2 px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem>
                <BreadcrumbPage>Submitted Form</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
      ...
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-foreground">
          Submitted Acknowledgement Forms
        </h1>
        <p className="text-sm text-muted-foreground mt-1">
          Review submitted acknowledgement forms and accept them to close cases.
        </p>
      </div>
      ...
      {showAcceptConfirm && formToAccept && (
        <ConfirmModal
          title="Accept Acknowledgement"
          description={`Are you sure you want to accept form ${formToAccept}? This will close the case.`}
          confirmText="Accept"
          cancelText="Cancel"
