...
export default function InProgressRequests() {
  const [inProgressSubmissions, setInProgressSubmissions] = useState<Submission[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedSubmission, setSelectedSubmission] = useState<Submission | null>(null);
  const [showViewModal, setShowViewModal] = useState(false);
  const [submissionToReject, setSubmissionToReject] = useState<Submission | null>(null);
  const [showRejectModal, setShowRejectModal] = useState(false);
  const [submissionToDisburse, setSubmissionToDisburse] = useState<Submission | null>(null);
  ...
  const fetchInProgressForms = async () => {
    try {
      ...
      const transformed = res.data.paymentInProgress.map((item: Record<string, unknown>) => ({
        ...(item as Submission),
  ...
  const filtered = useMemo(() => {
    let data = [...inProgressSubmissions];
    ...
    if (sortField && sortDir) {
      data.sort((a, b) => {
        if (sortField === "requested_amount" || sortField === "acceptedAmount") {
    ...
    return data;
  }, [inProgressSubmissions, search, sortField, sortDir]);
  ...
  return (
    <div className="flex min-h-full flex-col bg-background text-foreground">
        ...
        <div className="flex items-center gap-2 px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem>
                <BreadcrumbPage>Treasury Approval</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
      ...
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-foreground">Treasury Approval</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Review treasury-approved requests and move them to disbursement.
        </p>
      </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/40 hover:bg-muted/40">
                  ...
                  <TableHead className="cursor-pointer select-none" onClick={() => handleSort("requested_amount")}>Requested Amount<SortIcon field="requested_amount" {...sortProps} /></TableHead>
                  <TableHead className="cursor-pointer select-none" onClick={() => handleSort("acceptedAmount")}>Approved Amount<SortIcon field="acceptedAmount" {...sortProps} /></TableHead>
                  <TableHead className="cursor-pointer select-none" onClick={() => handleSort("mobile")}>Mobile Number<SortIcon field="mobile" {...sortProps} /></TableHead>
                  <TableHead className="cursor-pointer select-none" onClick={() => handleSort("alternateMobile")}>Alternate Number<SortIcon field="alternateMobile" {...sortProps} /></TableHead>
                  <TableHead className="cursor-pointer select-none" onClick={() => handleSort("submitted_at")}>Submitted At<SortIcon field="submitted_at" {...sortProps} /></TableHead>
                  <TableHead className="cursor-pointer select-none" onClick={() => handleSort("coordinatorName")}>UWF Member<SortIcon field="coordinatorName" {...sortProps} /></TableHead>
                  <TableHead>UWF Mobile</TableHead>
                  <TableHead className="cursor-pointer select-none" onClick={() => handleSort("region")}>Region<SortIcon field="region" {...sortProps} /></TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {paginated.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={11} className="h-32 text-center text-muted-foreground">
                      No treasury-approved forms found.
                    </TableCell>
                  </TableRow>
                ) : (
                  paginated.map((item) => (
                    <TableRow key={item.formId}>
                      <TableCell>
                        <div className="flex flex-wrap items-center gap-1.5">
                          ...
                          <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white text-xs px-2.5" onClick={() => { setSubmissionToDisburse(item); setShowDisburseModal(true); }}>Accept</Button>
                          <Button size="sm" variant="outline" className="border-red-200 text-red-600 hover:bg-red-500/10 hover:text-red-500 dark:border-red-800 text-xs px-2.5" onClick={() => { setSubmissionToReject(item); setShowRejectModal(true); }}>Revert Back</Button>
                        </div>
                      </TableCell>
                      ...
                      <TableCell>{item.acceptedAmount ?? "—"}</TableCell>
                      <TableCell className="text-sm">{item.mobile || "—"}</TableCell>
                      <TableCell className="text-sm">{item.alternateMobile || "—"}</TableCell>
                      <TableCell className="text-sm text-muted-foreground whitespace-nowrap">{formatDateTime(item.submitted_at)}</TableCell>
