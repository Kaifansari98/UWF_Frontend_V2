...
export default function DisbursedFormsPage() {
  ...
  const handleRevertBack = async () => {
    ...
    try {
      const res = await apiClient.put(`/submissions/revert-disbursement/${submissionToRevert.formId}`);
  ...
  const handleMarkAsDisbursed = async () => {
    ...
    try {
      const res = await apiClient.put(`/submissions/disburseARequest/${submissionToDisburse.formId}`);
      toast.success(res.data.message || "Marked as disbursed");
      fetchDisbursedForms();
    } catch (err: any) {
      toast.error(err.response?.data?.message || `Failed to disburse form ${submissionToDisburse.formId}`);
    } finally {
  ...
  return (
    <div className="flex min-h-full flex-col bg-background text-foreground">
        ...
        <div className="flex items-center gap-2 px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem>
                <BreadcrumbPage>Approved Cases</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
      ...
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-foreground">Approved Cases</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Review approved cases and send them forward for disbursement.
        </p>
      </div>
        ...
        {loading ? (
          ...
          <>
            <Table>
              ...
              <TableBody>
                {paginated.length === 0 ? (
                  <TableRow><TableCell colSpan={8} className="h-32 text-center text-muted-foreground">No approved cases found.</TableCell></TableRow>
                ) : (
                  paginated.map((item) => (
                    <TableRow key={item.formId}>
                      <TableCell>
                        <div className="flex flex-wrap items-center gap-1.5">
                          ...
                          <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white text-xs px-2.5" onClick={() => { setSubmissionToDisburse(item); setShowDisburseModal(true); }}>Send to Disbursement</Button>
                          <Button size="sm" variant="outline" className="border-red-200 text-red-600 hover:bg-red-500/10 hover:text-red-500 dark:border-red-800 text-xs px-2.5" onClick={() => { setSubmissionToRevert(item); setShowRevertModal(true); }}>Revert Back</Button>
                        </div>
                      </TableCell>
