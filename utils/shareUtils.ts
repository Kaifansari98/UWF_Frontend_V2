...
export const getBankInfoLetterMessage = ({
...
export const getBankInfoLetterWhatsAppURL = (params: BankInfoLetterParams) => {
  const encodedMessage = encodeURIComponent(getBankInfoLetterMessage(params));
  return `https://api.whatsapp.com/send?text=${encodedMessage}`;
};
